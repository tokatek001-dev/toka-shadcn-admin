...
describe('mergeMediaObject', () => {
  ...
  it('preserves snake_case extra keys on group media', () => {
    ...
    const merged = mergeMediaObject(existing, file, uploaded) as Record<
      string,
      unknown
    >
...
describe('uploadMedia', () => {
  it('posts multipart with Bearer token and returns the parsed result', async () => {
    ...
    const out = await uploadMedia(
      new File(['abcde'], 'a.png', { type: 'image/png' })
    )
  ...
  it('throws a descriptive error on HTTP failure', async () => {
    ...
    await expect(uploadMedia(new File(['x'], 'a.png'))).rejects.toThrow(
      /boom|500/
    )
  })
